...
uint32_t get_nr_line(std::string const &path) {
  ...
  uint32_t nr_line = 0;
  while (fgets(line, kMaxLineSize, f) != nullptr) ++nr_line;
...
void sort_problem(Problem &prob) {
  struct sort_by_v {
    bool operator()(Node const lhs, Node const rhs) { return lhs.v < rhs.v; }
  };
...
// Problem read_data(std::string const &dense_path, std::string const
// &sparse_path)
...
std::vector<std::string> argv_to_args(int const argc,
