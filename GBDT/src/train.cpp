...
std::string train_help() {
  return std::string(
      "usage: gbdt [<options>] <dense_validation_path> "
      "<sparse_validation_path> <dense_train_path> <sparse_train_path> "
      "<validation_output_path> <train_output_path>\n"
      "\n"
      "options:\n"
...
Option parse_option(std::vector<std::string> const &args) {
  ...
  if (i != argc - 4) throw std::invalid_argument("invalid command");
...
void write(Problem const &prob, GBDT const &gbdt, std::string const &path) {
  ...
  for (uint32_t i = 0; i < prob.nr_instance; ++i) {
    ...
    for (uint32_t t = 0; t < indices.size(); ++t) fprintf(f, " %d", indices[t]);
    fprintf(f, "\n");
  }
