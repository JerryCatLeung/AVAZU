...
void scan(Problem const &prob, std::vector<Location> const &locations,
          std::vector<Meta> const &metas0, std::vector<Defender> &defenders,
          uint32_t const offset, bool const forward) {
...
void scan_sparse(Problem const &prob, std::vector<Location> const &locations,
                 std::vector<Meta> const &metas0,
                 std::vector<Defender> &defenders, uint32_t const offset,
                 bool const forward) {
...
#pragma omp parallel for schedule(static)
  for (uint32_t i = 0; i < nr_instance; ++i) locations[i].r = R[i];
  for (uint32_t d = 0, offset = 1; d < max_depth; ++d, offset *= 2) {
    ...
    std::thread thread_b(scan, std::ref(prob), std::ref(locations),
                         std::ref(metas0), std::ref(defenders_inv), offset,
                         false);
...
std::pair<uint32_t, float> CART::predict(float const *const x) const {
  ...
  for (uint32_t d = 0; d <= max_depth; ++d) {
    ...
    if (tnode.feature == -1) return std::make_pair(tnode.idx, tnode.gamma);
...
float GBDT::predict(float const *const x) const {
  float s = bias;
  for (auto &tree : trees) s += tree.predict(x).second;
  return s;
}
...
std::vector<uint32_t> GBDT::get_indices(float const *const x) const {
  ...
  for (uint32_t t = 0; t < nr_tree; ++t) indices[t] = trees[t].predict(x).first;
  return indices;
}
