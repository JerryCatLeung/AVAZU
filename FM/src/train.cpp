...
std::string train_help() {
  return std::string(
      ...
      "\n"
      "<validation_path>.out will be automatically generated at the end of "
      "training\n"
      "\n"
      "options:\n"
      "-l <lambda>: set the regularization penalty\n"
      "-k <factor>: set the number of latent factors, which must be a multiple "
      "of 4\n"
...
Option parse_option(std::vector<std::string> const &args) {
  ...
  if (argc == 0) throw std::invalid_argument(train_help());
  ...
  for (; i < argc; ++i) {
    if (args[i].compare("-t") == 0) {
      if (i == argc - 1) throw std::invalid_argument("invalid command\n");
...
void init_model(Model &model) {
  ...
  for (uint32_t j = 0; j < model.nr_feature; ++j) {
    for (uint32_t f = 0; f < model.nr_field; ++f) {
      ...
      for (uint32_t d = nr_factor; d < 2 * nr_factor; ++d, ++w) *w = 0.5f;
    }
  }
}

void train(Problem const &Tr, Problem const &Va, Model &model,
           Option const &opt) {
  std::vector<uint32_t> order(Tr.Y.size());
  for (uint32_t i = 0; i < Tr.Y.size(); ++i) order[i] = i;
